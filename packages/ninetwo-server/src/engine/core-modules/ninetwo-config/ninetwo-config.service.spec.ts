...
jest.mock(
  'src/engine/core-modules/ninetwo-config/constants/config-variables-masking-config',
  () => ({
...
jest.mock(
  'src/engine/core-modules/ninetwo-config/utils/is-env-only-config-var.util',
  () => ({
    isEnvOnlyConfigVar: jest.fn(),
  }),
);

type NinetwoConfigServicePrivateProps = {
...
const setupTestModule = async (isDatabaseConfigEnabled = true) => {
  ...
  return {
    service: module.get<NinetwoConfigService>(NinetwoConfigService),
...
const setPrivateProps = (
  service: NinetwoConfigService,
  props: Partial<NinetwoConfigServicePrivateProps>,
) => {
...
describe('NinetwoConfigService', () => {
  let service: NinetwoConfigService;
