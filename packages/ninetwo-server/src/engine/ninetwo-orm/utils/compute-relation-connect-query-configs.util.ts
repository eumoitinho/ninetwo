...
import { getUniqueConstraintsFields, isDefined } from 'ninetwo-shared/utils';
...
import { type ConnectObject } from 'src/engine/ninetwo-orm/entity-manager/types/query-deep-partial-entity-with-nested-relation-fields.type';
import {
  type RelationConnectQueryConfig,
  type UniqueConstraintCondition,
} from 'src/engine/ninetwo-orm/entity-manager/types/relation-connect-query-config.type';
import { type RelationConnectQueryFieldsByEntityIndex } from 'src/engine/ninetwo-orm/entity-manager/types/relation-nested-query-fields-by-entity-index.type';
import {
  NinetwoORMException,
  NinetwoORMExceptionCode,
} from 'src/engine/ninetwo-orm/exceptions/ninetwo-orm.exception';
import { formatCompositeField } from 'src/engine/ninetwo-orm/utils/format-data.util';
...
const computeRecordToConnectCondition = (
  ...
  if (
    ...
    throw new NinetwoORMException(
      ...
      NinetwoORMExceptionCode.CONNECT_NOT_ALLOWED,
      {
...
const checkUniqueConstraintFullyPopulated = (
  ...
  if (!hasUniqueConstraintFieldFullyPopulated) {
    throw new NinetwoORMException(
      ...
      NinetwoORMExceptionCode.CONNECT_UNIQUE_CONSTRAINT_ERROR,
      {
