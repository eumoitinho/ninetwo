...
import { RELATION_NESTED_QUERY_KEYWORDS } from 'ninetwo-shared/constants';
import {
  type EntityTarget,
  type ObjectLiteral,
...
import { type QueryDeepPartialEntityWithNestedRelationFields } from 'src/engine/ninetwo-orm/entity-manager/types/query-deep-partial-entity-with-nested-relation-fields.type';
import { type RelationConnectQueryConfig } from 'src/engine/ninetwo-orm/entity-manager/types/relation-connect-query-config.type';
import {
  type RelationConnectQueryFieldsByEntityIndex,
  type RelationDisconnectQueryFieldsByEntityIndex,
} from 'src/engine/ninetwo-orm/entity-manager/types/relation-nested-query-fields-by-entity-index.type';
import {
  NinetwoORMException,
  NinetwoORMExceptionCode,
} from 'src/engine/ninetwo-orm/exceptions/ninetwo-orm.exception';
import { formatConnectRecordNotFoundErrorMessage } from 'src/engine/ninetwo-orm/relation-nested-queries/utils/formatConnectRecordNotFoundErrorMessage.util';
import { type WorkspaceSelectQueryBuilder } from 'src/engine/ninetwo-orm/repository/workspace-select-query-builder';
import { computeRelationConnectQueryConfigs } from 'src/engine/ninetwo-orm/utils/compute-relation-connect-query-configs.util';
import { createSqlWhereTupleInClause } from 'src/engine/ninetwo-orm/utils/create-sql-where-tuple-in-clause.utils';
import { extractNestedRelationFieldsByEntityIndex } from 'src/engine/ninetwo-orm/utils/extract-nested-relation-fields-by-entity-index.util';
import { getAssociatedRelationFieldName } from 'src/engine/ninetwo-orm/utils/get-associated-relation-field-name.util';
import { getObjectMetadataFromEntityTarget } from 'src/engine/ninetwo-orm/utils/get-object-metadata-from-entity-target.util';
import { getRecordToConnectFields } from 'src/engine/ninetwo-orm/utils/get-record-to-connect-fields.util';
        ...
        if (
          ...
          if (recordToConnect.length !== 1) {
            ...
            throw new NinetwoORMException(
              errorMessage,
              NinetwoORMExceptionCode.CONNECT_RECORD_NOT_FOUND,
              {
                userFriendlyMessage,
              },
            );
