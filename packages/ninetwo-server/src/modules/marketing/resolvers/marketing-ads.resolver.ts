...
export class MarketingAdsResolver {
  ...
  async getMarketingAdAccounts(
    ...
    if (connectedAccount.provider === 'google') {
      ...
      if (hasAdsScope) {
        const result = await this.googleAdsSyncService.fetchAccessibleCustomers(
          connectedAccountWithFreshTokens,
        );

        // Buscar contas já selecionadas do syncConfig
        const syncConfig = connectedAccount.syncConfig as {
          marketing?: { customerIds?: string[] };
        };
        const selectedAccounts = syncConfig?.marketing?.customerIds || [];
      ...
      throw new Error('Connected account does not have Google Ads permissions');
    }
  ...
  async configureMarketingAdAccounts(
    ...
    // Disparar job de sincronização automaticamente
  ...
  async getMarketingAnalyticsAccounts(
    ...
    if (connectedAccount.provider === 'google') {
      ...
      if (hasAnalyticsScope) {
        const result = await this.googleAnalyticsSyncService.fetchProperties(
          connectedAccountWithFreshTokens,
        );

        // Buscar propriedades já selecionadas do syncConfig
        const syncConfig = connectedAccount.syncConfig as {
          analytics?: { propertyIds?: string[] };
        };
        const selectedAccounts = syncConfig?.analytics?.propertyIds || [];
