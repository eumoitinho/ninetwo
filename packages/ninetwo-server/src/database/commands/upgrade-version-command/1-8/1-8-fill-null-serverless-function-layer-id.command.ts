...
@Command({
  ...
  description:
    'Fill null serverless function layer id to prepare to set serverlessFunctionId column NOT NULL',
})
export class FillNullServerlessFunctionLayerIdCommand extends ActiveOrSuspendedWorkspacesMigrationCommandRunner {
  ...
  constructor(
    ...
    protected readonly twentyORMGlobalManager: NinetwoORMGlobalManager,
  ...
  override async runOnWorkspace({
    ...
    this.logger.log(
      `Fill null serverlessFunction.serverlessFunctionLayerId for workspace ${workspaceId}`,
    );
    await this.fillNullServerlessFunctionLayerId();
  }

  private async fillNullServerlessFunctionLayerId() {
    const distinctWorkspaceIds = await this.serverlessFunctionRepository
      .createQueryBuilder('serverlessFunction')
      .select('DISTINCT serverlessFunction.workspaceId', 'workspaceId')
      .where('serverlessFunction.serverlessFunctionLayerId IS NULL')
      .getRawMany();

    for (const { workspaceId } of distinctWorkspaceIds) {
      const serverlessFunctionLayerId =
        await this.getOrInsertCommonLayer(workspaceId);

      await this.serverlessFunctionRepository
        .createQueryBuilder('serverlessFunction')
        .update()
        .set({ serverlessFunctionLayerId })
        .where('workspaceId = :workspaceId', { workspaceId })
        .andWhere('serverlessFunctionLayerId IS NULL')
        .execute();
    }
  }

  private async getOrInsertCommonLayer(workspaceId: string) {
    const commonServerlessFunctionLayers =
      await this.serverlessFunctionLayerRepository
        .createQueryBuilder('serverlessFunctionLayer')
        .select('serverlessFunctionLayer.id', 'id')
        .where('serverlessFunctionLayer.workspaceId = :workspaceId', {
          workspaceId,
        })
        .andWhere('serverlessFunctionLayer.checksum = :checksum', {
          checksum: COMMON_SERVERLESS_FUNCTION_LAYER_V1_CHECKSUM,
        })
        .getRawMany();

    if (commonServerlessFunctionLayers.length === 0) {
      const insertedServerlessFunctionLayers =
        await this.serverlessFunctionLayerRepository
          .createQueryBuilder()
          .insert()
          .into('serverlessFunctionLayer')
          .values({
            packageJson: COMMON_SERVERLESS_FUNCTION_LAYER_PACKAGE_JSON,
            yarnLock: COMMON_SERVERLESS_FUNCTION_LAYER_YARN_LOCK,
            checksum: COMMON_SERVERLESS_FUNCTION_LAYER_V1_CHECKSUM,
            workspaceId,
          })
          .returning('id')
      ...
      return insertedServerlessFunctionLayers.raw[0].id;
    }
...
const COMMON_SERVERLESS_FUNCTION_LAYER_PACKAGE_JSON = {
  dependencies: {
    uuid: '^10.0.0',
    axios: '^1.7.5',
    sharp: '^0.33.5',
...
const COMMON_SERVERLESS_FUNCTION_LAYER_YARN_LOCK = `# This file is generated by running "yarn install" inside your project.
