...
export default defineConfig(({ command, mode }) => {
  ...
  if (VITE_DISABLE_ESLINT_CHECKER !== 'true') {
    checkers['eslint'] = {
      lintCommand: 'eslint ../../packages/ninetwo-front --max-warnings 0',
      useFlatConfig: true,
    };
  }
  ...
  return {
    root: __dirname,
    cacheDir: '../../node_modules/.vite/packages/ninetwo-front',
    ...
    plugins: [
      ...
      tsconfigPaths({
        projects: [
          isBuildCommand ? 'tsconfig.build.json' : 'tsconfig.dev.json',
        ],
      }),
      svgr(),
      lingui({
